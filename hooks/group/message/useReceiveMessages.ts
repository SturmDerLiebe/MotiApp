...
export default function useReceiveMessageState(): [
    ...
    let [receivedMessageState, setReceivedMessageState] =
        useState<SocketStatus>(new SocketInitialLoading());
    ...
    async function startSocketConnection() {
        ...
        async function handleResponse() {
            ...
            if (RESPONSE.ok) {
                const RAW_MESSAGES: RawExistingMessageData[] =
                    await RESPONSE.json();
                const MESSAGES = transformRawToUIMessageList(RAW_MESSAGES);

                const DATE_SPLICED_DATA =
                    spliceChatMessageListWithDates(MESSAGES);

                setReceivedMessageState(
                    new SocketListSuccess(DATE_SPLICED_DATA),
                );
            ...
            function startFetchingNewMessages() {
                INTERVAL_REF.current = setInterval(async function () {
                    ...
                    if (RESPONSE.ok) {
                        ...
                        const DATA: RawExistingMessageData =
                            await RESPONSE.json();
        ...
        function handleError(error: unknown) {
            ...
            setReceivedMessageState(
                (currentState) =>
                    new SocketError(1008, currentState.mostRecentPayload),
            );
        }
    }
}
